...
#加循环
while True:
     shengfen = input("请您输入需要进行快递分拣的省份：")

     #print(shengfen)#测试一下
     #正式筛选，根据列表中元素还是列表，一样查询字符串中是否含有输入省份，因为查询列表的列表有点麻烦，直接将item的元素进行合并，''join(item)
     data_list = [item for item in data if shengfen in ''.join(item) ]
